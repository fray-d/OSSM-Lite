...
namespace streaming {
    ...
    static void startStreamingTask(void *pvParameters) {
        ...
        while (isInCorrectState()) {
            ...
            // settime is when the message was received. If we trust the source
            // we can reduce perceived lag by creating a buffer.
            if (USE_LATENCY_COMPENSATION) {
                ...
                ESP_LOGD("Streaming",
                         "Lagency Compensation - Lag: %d, Buffer: %d/%d", lag,
                         currentBuffer, mincomp);
                if (offset < 0) {
            ...
            // Map min/max percentages to stepper coordinates: maxStroke = 
            // range of motion; depth = offset of shallowest point from home
            int32_t maxStroke = std::abs((settings.maxPosition - settings.minPosition) /
                         100.0f * calibration.measuredStrokeSteps);
            int32_t depth = std::abs(settings.minPosition / 100.0f *
            ...
            if (speedLimit > 0 && accelLimit > 0) {
                targetPosition = (1 - (static_cast<float>(targetPositionTime.position) /
                          100.0f)) * maxStroke + depth;
                ...
                if (timeSeconds > 0.01f && distance > 1.0f) {
                    // Find max distance possible to travel given available
                    // time, max acceleration, and max speed.
                    int32_t maxDistance = accelLimit * pow(timeSeconds / 2, 2);
                    // This technically optimistic... the speed limit assumes
                    // perfect acceleration
                    maxDistance = min(maxDistance, int32_t(speedLimit * timeSeconds));
                    // if the distance asked for is greater then the maximum
                    // possible, reduce the ask. Is it what they asked for? No.
                    // Will they notice at these speeds? Hopefully not.
                    ...
                    // calculate what proportion of the move needs to be at max
                    // speed, if any.
                    float vt = requiredSpeed * timeSeconds;
                    float proportion = max(-((2 * distance - 2 * vt) / vt), 0.01f);
                    // calculate acceleration such that triangle or trapezoid is
                    // created depending on needs
                    uint32_t requiredAccel = requiredSpeed / (timeSeconds * proportion / 2);
                    // constrain just in case, but reducing the distance should
                    // mostly preven this.
