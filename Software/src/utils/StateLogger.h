...
struct StateLogger {
    ...
    [[gnu::used]] void log_process_event(const TEvent&) {
        ESP_LOGV("StateLogger", "%s", sml::aux::get_type_name<SM>());
        ...
        if (eventName.startsWith("boost::ext::sml")) {
            ESP_LOGV("StateLogger", "%s", eventName.c_str());
        } else {
            ESP_LOGD("StateLogger", "%s", eventName.c_str());
        }
    }
    ...
    [[gnu::used]] void log_guard(const TGuard&, const TEvent&, bool result) {
        ...
        ESP_LOGV("StateLogger", "%s: %s", resultString,
                 sml::aux::get_type_name<SM>());
        ESP_LOGD("StateLogger", "%s: %s, %s", resultString,
    ...
    [[gnu::used]] void log_state_change(const TSrcState& src,
                                        const TDstState& dst) {
        ESP_LOGV("StateLogger", "%s", sml::aux::get_type_name<SM>());
        ESP_LOGD("StateLogger", "%s -> %s", src.c_str(), dst.c_str());
    }
};
